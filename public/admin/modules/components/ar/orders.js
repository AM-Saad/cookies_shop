                ...
                <input type="hidden" name="itemId" value="${sales._id}">
                    <div class="inside-wrapper">
                        <div class="single-item_all_actions p-3 flex">
                            ...
                            <a href="/admin/orders/edit/${sales._id}" class="btn bg-blue">تعديل الطلب</a>

                           <a class="btn btn-danger single-item_actions delete-item">حذف الطلب <i class="fa fa-trash"></i></a>
                            </div>
                            <div class="single-item_info">
                                <div class="grid">
                                    <span>رقم الطلب:${sales.serialNo}</span>
                                    <span>حاله الطلب:${sales.status.text} </span>
                                </div>
                                <div class="grid">
                                    <span>تاريخ الطلب:${sales.date} </span>
                                    <span>تاريخ التوصيل:${sales.order_info.delivery_date} </span>
                                </div>
                            </div>
                            <div class="single-item_customer">
                                <div class="grid g-two bg-lightgray p-medium">
                                    <div>
                                         <p>ا/: <b>${sales.order_info.billing_details.first_name}</b></p>
                                         <p>رقم الموبايل: <b>${sales.order_info.billing_details.phone}</b></p>
                                    </div>
                                    <div>
                                    <p>المنطقه: <b>${sales.order_info.shipping_details.area.name}</b></p>
                                    <p>الشارع: <b>${sales.order_info.shipping_details.street}</b></p>
                                    <p>الدور: <b>${sales.order_info.shipping_details.floor}</b></p>
                                    <p>الشقه: <b>${sales.order_info.shipping_details.apartment}</b></p>
                                    </div>
                                  </div>
                            ...
                            <div class="bg-lightgray p-medium">
                                ...
                                <p class="p-medium  font-l"> سعر الشحن:${Math.floor(sales.order_info.shipping_details.price)}</p>
                                <p class="p-medium bg-dark c-g font-xl">المجموع الكلي:${Math.floor(sales.totalPrice)}</p>
                            </div>
